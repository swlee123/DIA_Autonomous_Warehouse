...
# mixing gym==0.22 to use this Monitor shit with gymnasium
...
class VecPyTorch(VecEnvWrapper):
    ...
    def reset(self):
        ...
        # debugging code 
        # Flatten obs[i] if it is a tuple of arrays
        processed_obs = []
        for o in obs:
            if isinstance(o, tuple):  # flatten tuple of arrays
                o = np.stack(o).astype(np.float32)  

            elif isinstance(o, np.ndarray) and o.dtype == object:
                # Probably array([ (array1, array2) ]) — need to flatten
                flat = np.stack(o[0]).astype(np.float32)
                o = flat
            processed_obs.append(torch.from_numpy(o).to(self.device))
        print("Obs after preprocessing:")
        for i, o in enumerate(processed_obs):
